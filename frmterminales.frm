...
Begin VB.Form frmTerminales 
   ...
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   30
      TabIndex        =   3
      Tag             =   "Descripción|T|N|||incidencias|nominci|||"
      Top             =   4920
      Visible         =   0   'False
      Width           =   2835
   End
   Begin VB.ComboBox Combo1 
      ...
      Height          =   360
      Index           =   1
      ItemData        =   "frmterminales.frx":000C
      Left            =   7320
      List            =   "frmterminales.frx":000E
      Style           =   2  'Dropdown List
      TabIndex        =   4
      Tag             =   "Ti|N|N|0||incidencias|Continuada|||"
      Top             =   4920
      Visible         =   0   'False
      Width           =   1335
   End
   Begin VB.ComboBox Combo1 
      ...
      Height          =   360
      Index           =   0
      ItemData        =   "frmterminales.frx":0010
      Left            =   840
      List            =   "frmterminales.frx":0012
      Style           =   2  'Dropdown List
      TabIndex        =   1
      Tag             =   "Ti|N|N|0||incidencias|Continuada|||"
      Top             =   4920
   ...
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   6
      TabIndex        =   0
      Tag             =   "Código|N|N|0||Incidencias|idinci|0|S|"
      Top             =   4920
      Visible         =   0   'False
      Width           =   800
   End
   Begin MSDataGridLib.DataGrid DataGrid1 
      Bindings        =   "frmterminales.frx":0014
      Height          =   6195
      Left            =   120
      TabIndex        =   10
      Top             =   600
   ...
   End
End
Attribute VB_Name = "frmTerminales"
...
Dim anc As Single
    ...
    If (DatosADevolverBusqueda <> "") And NuevoCodigo <> "" Then
        NumF = NuevoCodigo
    Else
        NumF = SugerirCodigoSiguienteStr("terminales", "id")
    End If
    '********************************************************************
...
Private Sub BotonModificar()
    ...
    CadB = DBLet(adodc1.Recordset!tipo, "T")
    If CadB = "" Then CadB = "N/D"
    For i = 0 To Combo1(0).ListCount - 1
        If Combo1(0).List(i) = CadB Then
...
'Dim temp As Boolean
    ...
    If CStr(adodc1.Recordset!Id) = "0" Then
        MsgBox "Terminal 0 no se puede borrar", vbExclamation
        Exit Sub
    End If
    ...
    SQL = "¿Seguro que desea eliminar el terminal?"
    ...
    If MsgBox(SQL, vbQuestion + vbYesNo) = vbYes Then
        ...
        SQL = "Delete from terminales where id=" & adodc1.Recordset!Id
        conn.Execute SQL
...
'    chkVistaPrevia.Value = CheckValueLeer(Name)
    ...
    CadB = "select desctipoterminal descripcion ,codTipoTerminal id from TerminalTipo order by 1"
    CargaComboTabla CadB, Me.Combo1(0), False
    
    CadB = "select   descripcion   ,codarea id from areas order by 1"
    CargaComboTabla CadB, Me.Combo1(1), True
    
    '****************** canviar la consulta *********************************+
    CadenaConsulta = "SELECT id,tipo,idterminal,terminales.descripcion,areas.descripcion,area"
    CadenaConsulta = CadenaConsulta & " FROM terminales , areas WHERE area=codarea"
    '************************************************************************
...
'    adodc1.ConnectionString = Conn
    ...
    tots = "S|txtAux(0)|T|Id|700|;S|Combo1(0)|C|Tipo|1250|;S|txtAux(1)|T|Identificador|1600|"
    tots = tots & ";S|txtAux(2)|T|Descripcion|3100|;S|Combo1(1)|C|Zona-area|2100|;N||||0|;"
...
Dim B As Boolean
    ...
    If Modo = 3 Then
        ...
        'la N es pa dir que es numeric
         ...
         Datos = DevuelveDesdeBD("id", "terminales", "id", txtAux(0).Text, "N")
         If Datos <> "" Then
            MsgBox "Ya existe el terminal: " & txtAux(0).Text, vbExclamation
            DatosOk = False
            PonerFoco txtAux(0)
            Exit Function
        End If
...
Dim i As Integer
    ...
    cadSeleccion = "reloj"
    For i = 1 To 2
        C = RecuperaValor("entradafichajes|entradamarcajes|", i)
        cadSeleccion = DevuelveDesdeBD("count(*)", C, cadSeleccion, CStr(adodc1.Recordset!Id))
...
Private Function InsertaModifica() As Boolean
    InsertaModifica = False
    
    CadB = ""
    For NumRegElim = 0 To txtAux.Count - 1
        txtAux(NumRegElim).Text = Trim(txtAux(NumRegElim).Text)
        If txtAux(NumRegElim).Text = "" Then CadB = "N"
        If NumRegElim < 2 Then
            If Combo1(NumRegElim).ListIndex < 0 Then CadB = "N"
        End If
    Next
    ...
    If Modo = 3 Then
        CadB = "INSERT INTO terminales(id,tipo,idterminal,descripcion,area) VALUES (" & txtAux(0).Text & ","
        CadB = CadB & DBSet(Combo1(0).Text, "T") & "," & DBSet(txtAux(1).Text, "T") & ","
        CadB = CadB & DBSet(txtAux(2).Text, "T") & "," & Combo1(1).ItemData(Combo1(1).ListIndex) & ")"
    
    Else
        CadB = "UPDATE terminales SET tipo=" & DBSet(Combo1(0).Text, "T")
        CadB = CadB & ", idterminal = " & DBSet(txtAux(1).Text, "T")
        CadB = CadB & ", descripcion = " & DBSet(txtAux(2).Text, "T")
        CadB = CadB & ", area = " & Combo1(1).ItemData(Combo1(1).ListIndex)
        CadB = CadB & " WHERE id =" & adodc1.Recordset!Id
    End If
    If EjecutaSQL(CadB) Then InsertaModifica = True
    CadB = ""
End Function
