...
Dim Cont As Integer
    ...
    Else
        ...
        If MsgBox(SQL, vbQuestion + vbYesNoCancel) = vbYes Then
            SQL = "Delete FROM Usuarios.usuarioempresaPres  WHERE codusu =" & ListView1.SelectedItem.Text
...
Private Sub InsertarEmpresa(Empresa As Integer)
    SQL = "INSERT INTO Usuarios.usuarioempresaPres (codusu,codempre) VALUES ("
    ...
    If Index = 0 Then
        ...
        Else
            ...
            Else
                ...
                'es decir, le estoy asignando su nivelpres de contabilidad
...
On Error GoTo EInsertarModificar
    ...
    If UCase(Label6.Caption) = "NUEVO" Then
        ...
        SQL = "INSERT INTO Usuarios.usuarios (codusu, nomusu,  nivelpres, login, passwordpropio,dirfich) VALUES ("
        SQL = SQL & i
...
Private Sub cmdUsu_Click(Index As Integer)
    ...
    Case 2, 3
        ...
        Else
            SQL = "Al usuario " & ListView1.SelectedItem.SubItems(1) & " no le estará permitido el acceso al programa Aripres." & vbCrLf
            SQL = SQL & vbCrLf & "                              ¿Desea continuar?"
            If MsgBox(SQL, vbQuestion + vbYesNo) = vbNo Then Exit Sub
            SQL = "UPDATE Usuarios.usuarios SET nivelpres = -1 WHERE codusu = " & ListView1.SelectedItem.Text
        End If
        Screen.MousePointer = vbHourglass
            Conn.Execute SQL
...
Private Sub Form_Activate()
    If PrimeraVez Then
        PrimeraVez = False
        Me.ListView1.SmallIcons = frmMain.ImageList2
        Me.ListView2.SmallIcons = frmMain.ImageList2
        CargaUsuarios
    End If
...
Dim itm As ListItem
    ...
    SQL = "Select * from Usuarios.Usuarios where nivelpres >=0 and codusu > 0 order by codusu"
    miRsAux.Open SQL, Conn, adOpenForwardOnly, adLockPessimistic, adCmdText
    While Not miRsAux.EOF
        ...
        SQL = miRsAux!nivelpres & "|" & miRsAux!nomusu & "|"
        itm.Tag = SQL
        'Sig
        miRsAux.MoveNext
    Wend
            ...
            If Not miRsAux.EOF Then
                ...
                Else
                    ...
                    Else
                        ...
                        SQL = SQL & "¿Desea agragarlo como usuario al control de presencia?"
...
Private Sub LeerEditorMenus()
    ...
    SQL = "Select count(*) from usuarios.appmenus where aplicacion='Pres'"
    Set miRsAux = New ADODB.Recordset
    miRsAux.Open SQL, Conn, adOpenForwardOnly, adLockPessimistic, adCmdText
    If Not miRsAux.EOF Then
...
Dim J As Integer
    ...
    SQL = "Select * from usuarios.appmenus where aplicacion='Pres'"
